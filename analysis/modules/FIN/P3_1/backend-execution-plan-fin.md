...
Validations: uniqueness of code (QR-FIN-046, DB `UQ_FIN_DIMENSION_CODE` friendly-checked at service layer)
...
marker is the single one the API-FIN-020 builder reads; all four lookup codes validated
via XM-FIN-001
...
Header coherence (RULE-FIN-017) runs FIRST and fail-fast: the submitted `periodId` must
...
Validations: uniqueness of code (QR-FIN-047)
...
Closed → resolve the successor year (QR-FIN-048) and the Retained Earnings account
(QR-FIN-045) → compute closing
balances (QR-FIN-041) →
...
SEC's ENT-SEC-001 (User) + its role/permission grant tables. `FinSeparationOfDutiesService`
...
|---|---|---|---|---|---|
| FIN_ACCOUNTS | ✓ `PERM_FIN_ACCOUNTS_VIEW` (API-FIN-001) | ✓ `PERM_FIN_ACCOUNTS_CREATE` (API-FIN-002) | ✓ `PERM_FIN_ACCOUNTS_UPDATE` (API-FIN-003, and API-FIN-004 deactivate) | — | — |
| FIN_DIMENSIONS | ✓ `PERM_FIN_DIMENSIONS_VIEW` (API-FIN-005,008) | ✓ `PERM_FIN_DIMENSIONS_CREATE` (API-FIN-006,007) | ✓ `PERM_FIN_DIMENSIONS_UPDATE` (API-FIN-035, deactivate a dimension VALUE — added by V28) | — | — |
| FIN_RULES | ✓ `PERM_FIN_RULES_VIEW` (API-FIN-009) | ✓ `PERM_FIN_RULES_CREATE` (API-FIN-010) | ✓ `PERM_FIN_RULES_UPDATE` (API-FIN-011, add line; and API-FIN-034, deactivate rule) | — | — |
| FIN_RECURRING_TEMPLATES | ✓ `PERM_FIN_RECURRING_TEMPLATES_VIEW` (API-FIN-012) | ✓ `PERM_FIN_RECURRING_TEMPLATES_CREATE` (API-FIN-013) | ✓ `PERM_FIN_RECURRING_TEMPLATES_UPDATE` (API-FIN-014, run; and API-FIN-036, deactivate template) | — | — |
| FIN_ALLOCATION_RULES | ✓ `PERM_FIN_ALLOCATION_RULES_VIEW` (API-FIN-015) | ✓ `PERM_FIN_ALLOCATION_RULES_CREATE` (API-FIN-016) | ✓ `PERM_FIN_ALLOCATION_RULES_UPDATE` (API-FIN-017, run; and API-FIN-037, deactivate rule) | — | — |
| FIN_JOURNAL_ENTRIES | ✓ `PERM_FIN_JOURNAL_ENTRIES_VIEW` (API-FIN-018,022) | ✓ `PERM_FIN_JOURNAL_ENTRIES_CREATE` (API-FIN-019,020) | — | — | Reverse `PERM_FIN_JOURNAL_ENTRIES_REVERSE` (API-FIN-021) |
| FIN_PERIODS | ✓ `PERM_FIN_PERIODS_VIEW` (API-FIN-033, search periods — and still the gateway, see below) | ✓ `PERM_FIN_PERIODS_CREATE` (API-FIN-023, year) | ✓ `PERM_FIN_PERIODS_UPDATE` (API-FIN-024,025) | — | Close-approve `PERM_FIN_PERIODS_CLOSE_APPROVE` (API-FIN-026,027 — the distinct permission RULE-FIN-015 requires) |
| FIN_ACCOUNT_LEDGER | ✓ `PERM_FIN_ACCOUNT_LEDGER_VIEW` (API-FIN-028) | — | — | — | — |
| FIN_TRIAL_BALANCE | ✓ `PERM_FIN_TRIAL_BALANCE_VIEW` (API-FIN-029) | — | — | — | — |
| FIN_BALANCE_SHEET | ✓ `PERM_FIN_BALANCE_SHEET_VIEW` (API-FIN-030) | — | — | — | — |
| FIN_INCOME_STATEMENT | ✓ `PERM_FIN_INCOME_STATEMENT_VIEW` (API-FIN-031) | — | — | — | — |
| FIN_DIMENSION_REPORTS | ✓ `PERM_FIN_DIMENSION_REPORTS_VIEW` (API-FIN-032) | — | — | — | — |
...
| ~~FIN service-unavailable (never declared)~~ STRUCK | PLATFORM-STD (MDL unreachable) | — | — | this row's code was originally shaped `{MOD}-{http}` with http 503 — never a declared platform status (`stack.backend.api.http_statuses` has no entry for it) and never a `FinErrorCodes` constant. Struck rather than spelled out here, so the row's own label no longer reads as a live catalog code. Genuinely unreachable: XM-FIN-001 is in-process `MdlLookupApi` injection, so there is no network hop to fail; an invalid code raises `FIN-400-INVALID-LOOKUP`, anything else falls through to `FIN-500` | — | — |
...
MIRROR DIVERGENCE ✓ CLOSED 2026-09-17. The two supposedly-mirrored copies of the API-FIN-019
block were not identical: SVC-API-CRUD.md carried a `Numbering:` paragraph (docNo format,
the VARCHAR(30) width argument, the per-fiscalYearId counter,
UQ_FIN_JOURNAL_ENTRY_YEAR_DOCNO, and the explicit "no platform numbering engine exists in
this repo") that this file's copy of the same block, addressed by its own
`API:API-FIN-019` START/END marker pair, did not — no fact contradicted the other, this
copy simply carried less detail. That paragraph is now copied in verbatim here and the
Orchestration line's inline docNo mention removed to match (the format lives in
`Numbering:` alone, cited by id, never restated). Re-verify byte-equality of the two
blocks' bodies before recording this closed again in a later revision.
