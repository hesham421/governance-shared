...
Inputs : srs (v1), prd (v1, APPROVED), api-docs (v1, re-fetched), registry-srs (v1),
         registry-exec-be (v1)
Screens: 2 — SCR-MDL-001..002 (one per SRS Part B screen requirement)
Flows  : 5 · ADRs : ADR-MDL-002 … ADR-MDL-008 (all ACCEPTED, all non-breaking;
         ADR-MDL-001 superseded by ADR-MDL-008)
══════════════════════════════════════════════════════════════════
...
Sequence  : (menu) → SCR-MDL-001 type list → [New] → the type form beside the list
            ...
            → save (one save, the type alone) → the type appears in the list and its (empty)
              value pane opens
...
Sequence  : SCR-MDL-001 → [type selected] → its values in the detail pane, ordered by sort order
            → [New value] → the value entry opens as the detail pane's second level, at its own
              address → code, both names, sort order → save (one save, one value)
              → the value list refreshes and the second level closes
...
consumer no longer receives, which is exactly what the consumer read hides. The value entry
never appears inside the type form: the two write different records through different
endpoints, and a screen with one save per entry cannot lose one of them to the other.
...
field and the form shows whatever the last drag produced. The drag is its own explicit submit
on the list and belongs to no form, so it adds no second save to any surface.
...
B3 every field/permission on a screen exists in the SRS
   → fields copied from SRS A3 per owning ENT and reconciled against the api-docs DTOs in
     ui-ux-spec-mdl.md; permission names taken from the SRS Access summary and the published
     api-docs, never minted here. Extra removed: none. Missing added: none. One divergence, on
...
RESULT  reconciled 2 · reworked 1 (SCR-MDL-001's isActiveFl, per B3 above) ·
        ADRs ADR-MDL-002, ADR-MDL-003, ADR-MDL-004, ADR-MDL-005, ADR-MDL-006, ADR-MDL-007,
        ADR-MDL-008 (all ACCEPTED, all non-breaking; ADR-MDL-008 is this run's own — the
        published api-docs carries no governance id, so Part B binds by verb and path —
        and it supersedes ADR-MDL-001)
```
══════════════════════════════════════════════════════════════════
