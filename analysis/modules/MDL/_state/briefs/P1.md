...
- `governance-shared/analysis/modules/MDL/P1/registry-srs-mdl.md` (registry)
...
- **C4** PRD → SRS (human PRD approval in between): C4.1 exists {'artifact': 'prd'} [CRITICAL]; C4.2 gate-approved {'gate': 'prd-approval'} [CRITICAL]; C4.3 ids-owned {'stage': 'P0.5'} [CRITICAL]; C4.4 traces {'from': 'US', 'to': ['POL'], 'min': 1} [MAJOR]; C4.5 no-questions {'stage': 'P0.5'} [CRITICAL]; C4.6 languages {'stage': 'P0.5'} [MAJOR]; C4.7 glossary {'artifact': ['prd'], 'glossary': 'vocabulary.glossary', 'synonyms': 'vocabulary.glossary_synonyms'} [MINOR]
...
RULE-MDL-[SEQ] — [short name]
  ...
  Data source: ENT-MDL-[SEQ].[field] [, …]             (mandatory — see below)
...
*reads to decide*. Every field named here is an `ENT-MDL-[SEQ].[field]` that A3
...
```
  Data source: ENT-MDL-[SEQ].[field]        ← the declaration surface exists: name it,
...
ADR file : analysis/decisions/MDL/ADR-{MOD}-{seq:03d}.md
...
- [ ] Every RULE carries a `Data source` that either names `ENT-MDL-[SEQ].[field]` values A3 declares, or is the explicit `DEFERRED — no declaration surface in this version` marker (§5).
