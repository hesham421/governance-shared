# BRIEF — stage `P1` (SRS) · module NOTE · v1 · profile `erp`
...
- `erp/modules/NOTE/P1/srs-note.md`
- `erp/modules/NOTE/P1/registry-srs-note.md` (registry)
...
|---|---|---|
| PRIVATE | fully owned by this module | `ENT-NOTE-[SEQ]` — [name] — PRIVATE |
| SHARED (owner) | other modules consume read-only | `ENT-NOTE-[SEQ]` — [name] — SHARED (owner) |
...
RULE-NOTE-[SEQ] — [short name]
  ...
  Traces     : REQ-NOTE-[SEQ] [, …]                     (≥ 1, mandatory)
  Data source: ENT-NOTE-[SEQ].[field] [, …]             (mandatory — see below)
...
*reads to decide*. Every field named here is an `ENT-NOTE-[SEQ].[field]` that A3
...
```
  Data source: ENT-NOTE-[SEQ].[field]        ← the declaration surface exists: name it,
...
```
SCR-REQ-NOTE-[SEQ] — [screen name in each of ar/en]
...
ADR file : erp/decisions/NOTE/ADR-{MOD}-{seq:03d}.md
...
### REQ-NOTE-001 — [name]        (record §4)
#### AC-NOTE-001 … (record §4.1, ≥ 1 per REQ)
(repeat per requirement)

## A5 — Business rules
### RULE-NOTE-001 — [name]       (record §5)
...
Single-source rule: PART A defines; PART B references by ID ("applies RULE-NOTE-003").
...
Event         : "P1 completed: NOTE v1 — [counts]"
```
...
- [ ] Every RULE carries a `Data source` that either names `ENT-NOTE-[SEQ].[field]` values A3 declares, or is the explicit `DEFERRED — no declaration surface in this version` marker (§5).
